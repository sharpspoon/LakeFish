...
class WeatherScraper:
    ...
    def jday_calc(self, i):
        # Value used in calculation of solar raditaion
        date_string = self.month + "/" + str(i) + "/" + self.year
        date = datetime.strptime(date_string, "%m/%d/%Y")
        # TEXT(W2-DATE(YEAR(W2),1,0),"000") -> calculates days from start of year
        days = date.strftime('%j')
        # RIGHT(YEAR(W2),2) -> Just grabs the year number
        year = date_string[-2:]
        formated = year + days
        return formated

    def solar_rad_cal(self, solar_cov, i):
        ...
        if ao < 0:
            ...
            phis = 24 * (2.044 * ao + 0.1296 * ao**2 - 1.941 *
                         0.001 * ao**3 + 7.591 * 0.000001 * ao**4) * 0.1314
