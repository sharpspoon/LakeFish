...
CFX
      ...
      COMMON/NEW/NYEAR,KYEAR(30),FDTH(5),NDEPTH,NTDY(30)
      COMMON/CHLAP/NCDAY(40,2),GCHLA(40,2),ICHLA,IFIELD,CHLTOP(30),
     +  CHLBOT(30)
     ...
     + HSCN(3),HSCNH(3),XNDEL(3),IDIATOM,CHLMEAN(30),CHLMAX,SDY(30)
	...
	COMMON/LOCATION/ISTATE,ISTATION,SLAT,SLON,YRAIR(61,16),ELEV(61,16)
     ...
     +            	TBAK(40),DOBAK(40)
      COMMON/FWEA/FTAR(12),FDEW(12),FWIN(12),FTCL(12),
     + FPRE(12),FSOL(12)
...
C96** Latitude, longitude of stations and No. station in a state
      ...
      CHARACTER*80 TITLE, RUNPATH, METERPATH
      CHARACTER*16 DIN,MET,FLO,TAPE8,TAPE1,TAPE9,TAPE7
	CHARACTER*1 FCO2
      CHARACTER*1 T8(16),XS,NUM(10),T9(16),T7(16),PATH(80),FFILE(100),
     ...
     &         FILE26(16),FILE25(16),FILE36(16),FILE4(16),PATHM(80)
...
c      READ(17,'(A)') RUNPATH
c      READ(17,'(A)') METERPATH
CFX These two parameters are passed into Fortran Program by using "Process"
      READ(*,'(A)') RUNPATH
      READ(*,'(A)') METERPATH
...
CFX*** "p" indicate for past climate conditions
		T7(1)='P'
          T9(1)='P'

C Read future climate scenario - CCC GCM !
		IF(FCO2.EQ.'y'.OR.FCO2.EQ.'Y') THEN

CFX*** "f" indicate for FUTURE climate conditions
		 T7(1)='F'
	     T9(1)='F'
          ENDIF
...
C *** OPEN FILE "TAPE9" FOR WATER TEMPERATURE SIMULATION
...
C *** OPEN FILE "FD-CN.SDF" - IS NOT USING RIGHT NOW
C     CALL MAKEFILE(PATH,FILE58,FFILE)
C     OPEN (58,FILE=FILENAME)
...
C FTAR(1) is increment of January air temperature
C for 2xCO2 GCM climate conditions
...
C***** Call LAKE routine to READ geometry data *** 
      ...
      CALL START(ST,S,FT,ISTART,INFLOW,ITER,II,TSLAV,PATH)
...
C*** Unnecessary, ZMAX value from Common Block /VOL/
C      ZMAX=ST-DBL
...
C20** NDAYS stands for number of data with field data
      NDAYS=1
      MP=0
      IPRNT(1)=1
      KDAYS=0
...
CFX***Specify total chlorophyll-a concentration in mg/L
          CALL PTOTALS(MAXMTH,MXDAY,CHLMAX,CHLMEAN(NUMY),SDY(NUMY),NUMY)
...
C Corrections for future climate sceanrios 
C based on CCC GCM output (1990)
C Canadian Climate Centre, Atmospheric Environment Centre
...
C Directly use given specific humidity from CCC GCM
C RH -- Relative humidity, SPH - Specific humidity
C Compute specific humidity for the past climate condition
        PO=1.0133E+5
        EA=611.0*EXP(17.3*TDEW(K)/(237.2+TDEW(K)))
        SPH=0.622*EA/PO*FDEW(MONTH)

C For GISS climate scenario where RH is given !
...
C EA for future climate scenario !
        EA=SPH*PO/0.622
        EAV=EA/611.0
        TDEW(K)=237.2*ALOG(EAV)/(17.3-ALOG(EAV))

C Set condition for dew point temperatures
...
C                                                            C
C************************************************************C
      SUBROUTINE START(ST,S,FT,ISTART,INFLOW,ITER,II8,TSLAV,PATH)
...
CFX   USING CHIAI AND CHLHY(I)
	...
	COMMON/LOCATION/ISTATE,ISTATION,SLAT,SLON,YRAIR(61,16),ELEV(61,16)
     ...
     +            	TBAK(40),DOBAK(40)
      CHARACTER*16 DIN,MET,FLO,TAPE8,TAPE1
      CHARACTER*1 T1(16),PATH(80),FFILE(100),
...
C20** ICHLA=1/0 is having YES/NO chl-a data
...
C20** NDEPTH IS CUURENTLY FIXED AS 5
...
C20** Compute average SDY from chlorophyll-a concentration
...
C20**SDY(IXY) will be zero for those years without field data First!
C20**CHLTOP is annual mean Chlorophyll-a cconcentration in eplimnion;
C20**CHLBOT is annual mean Chlorophyll-a cconcentration in Hypolimnion;
	   SDY(IXY)=0.0
         CHLTOP(IXY)=0.0
	   CHLBOT(IXY)=0.0
...
C20** SDAVG, TOPCHL, BOTCHL are AVERAGE Secchi dish depth, eplimion
C20** and hypolimnion chlorophyll-a concentration for all available data.
...
C20**SDY(IXY)=0 for years without field data will be SET as average SD,
C20**chlorophyll-a concentrations of those years with field data.
   
          SDAVG=SDAVG/FLOAT(NDAYO)
	    TOPCHL=TOPCHL/FLOAT(NDAYO)
	    BOTCHL=BOTCHL/FLOAT(NDAYO)
          
   	    IXY=1
         DO 146 IX=MYEAR,FYEAR
...
C2002** (1) If there is not field data for Temp/DO wether you set
C2002** yes or no for Chla data or (2)if you have field data
C2002** for Temp/DO, but there is no field data for Chla.

	IF(IFIELD.NE.1.OR.ICHLA.NE.1) THEN 

C20** If there are no field data for chlorophyll-a, you input an 
C20** annual mean chl-a concentration year by year (CHLMEAN), and
C20** use a default seasonal pattern, SDY - Secchi Disk Depth
C20** FOR EVERY YEAR! 
        DO 147 IK=1,NYTOT 
	   READ (7,*) CHLMEAN(IK),SDY(IK)
 147	  CONTINUE
	ENDIF
...
CFX*** JUDF is the Julian day of first simulation as constant
      JUDF=JDY
C20** Reset NYEAR = 1 STARTING AGAIN!
      NYEAR=1
      CHLEP(1)=CHLA2(1,1)      
      CHLHY(1)=CHLEP(1)
	SD=SDY(1)      
...
C                                                            C
C************************************************************C
      SUBROUTINE PTOTALS(MAXMTH,MXDAY,CHLMAX,CHLMEANY,SDYY,NUMY)
C*****
...
C are SD = 1.2, 2.5, 4.5 m, respectively, for regional lakes (1997).
...
C20** If we have field data WITH chl-a concentrations!!
      IF (IFIELD.EQ.1.AND.ICHLA.EQ.1) THEN
 
C
C Step seasonal distribution/function for Chla concentration
C CHLTOP OR CHLBOT are mean chlas year by year
C NUMY is the number of simulation years starting 1 in the first year.
      IF(JZ.LE.NCDAY(IST+1,M)) THEN
       ...
       CHLATOT01=(1.0+RATIO)*CHLTOP(NUMY)
	 CHLATOT02=(1.0+RATIO)*CHLBOT(NUMY)
      ENDIF
	
	IF(NUMY.EQ.2) THEN
c	WRITE(*,*) JZ,CHLATOT01,CHLATOT02
	ENDIF
...
C      IF(MYEAR.EQ.KYEAR(1)) THEN
C        MFIRST=1
C        MLAST=NTDY(1)
C      ENDIF

C      IF(KDY.EQ.101) THEN
C        IF(MYEAR.NE.KYEAR(NYEAR)) THEN
C          MFIRST=KI-NTDY(NYEAR)
C          MLAST=MFIRST+NTDY(NYEAR)-1
C          NDAYS=KI-NTDY(NYEAR)
C        ELSE
C          MFIRST=KI
C          MLAST=MFIRST+NTDY(NYEAR)-1
C        ENDIF
...
C      IF(THICE.GT.0.0) THEN
C        CHLATOT(1)=CHLEP(1)/2.0
C        CHLABOT=CHLHY(1)/2.0
C        GOTO 656
C      ENDIF   

C*Use constant Chl-a before the last field data!
C      IF(KDY.LE.NPRNT(MFIRST).AND.THICE.EQ.0.0) THEN
C        CHLATOT(1)=CHLEP(MFIRST+1)
C        CHLABOT=CHLHY(MFIRST+1)
C        GOTO 656
C      ENDIF  

C*Use constant Chl-a after the last field data!
C      IF(KDY.GT.NPRNT(MLAST).AND.THICE.EQ.0.0) THEN
C        CHLATOT(1)=CHLEP(MLAST+1)
C        CHLABOT=CHLHY(MLAST+1)
C        GOTO 656
C      ENDIF  
       
C*The chlorophyll values will be the mean value of nearest field data
C      IF(KDY.LE.NPRNT(KI).AND.KDY.GT.NPRNT(KI-1)) THEN
C        CHLATOT(1)=(CHLEP(KI)+CHLEP(KI+1))/2.0
C        CHLABOT=(CHLHY(KI)+CHLHY(KI+1))/2.0
C        GOTO 656
C      ENDIF
...
C      IF(MYEAR.NE.KYEAR(NYEAR).AND.KDY.EQ.NPRNT(KI)) NDAYS=NDAYS+1

C 700  DO 108 I=2,MBOT
C        CHLATOT(I)=CHLATOT(1)
C        IF(Z(I).GT.Z(ILAY)) THEN
C          CHLATOT(I)=CHLABOT            
C        ENDIF
C 108  CONTINUE
...
C
C Step seasonal distribution/function for Chla concentration
C20 CHLMEANY is mean chla year by year
C
      IF(JZ.LE.NCDAY(IST+1,M)) THEN
       DDY=JZ-NCDAY(IST,M)
       DDT=NCDAY(IST+1,M)-NCDAY(IST,M)
...
C*************************************************
	SUBROUTINE MAKEFILE(PATH,TMPFILE,FFILE)
      CHARACTER*1 PATH(80),TMPFILE(16),FFILE(100)
...
C**************************************************************
C
C   Read the weather data for MINLAKE95 from CCC GCM output
C   (2xCO2 and 1xCO2 climate scenarios)
C   By Xing Fang (June, 1995)
C
C*************************************************************
       SUBROUTINE FSCENA(KNAME,SLON,SLAT)
	 PARAMETER(IDM=30,JDM=19,IJDM=IDM*JDM)
       DIMENSION DAT(IDM,JDM),DDD(IJDM),SD(4)
       DIMENSION ALON(30),ALAT(19),RES(12,7)
       CHARACTER NLAB*80,NBF*80,LLAB*80,IFID*4,ITYP*4,LTYP*4
       CHARACTER KNAME*20,PIN(7)*4,PTL(7)*30,SBF*80
       EQUIVALENCE (DAT,DDD)
       DATA ALAT/20.14,24.12,27.83,31.54,35.26,38.97,42.68,
     +  46.39,50.10,53.81,57.52,61.23,64.94,68.65,
     +  72.36,76.07,79.78,83.48,87.16/
       OPEN(376,FILE='D:\Inetpub\wwwroot\lake\FortranModel\co1.dat')
       ICO=1
...
C DETERMINE THE NEAREST GRID POINT !        
 109   SD(1)=SQRT((SLON-ALON(IA))**2+(SLAT-ALAT(IB))**2)
       SD(2)=SQRT((SLON-ALON(IA))**2+(SLAT-ALAT(IB+1))**2)
       SD(3)=SQRT((SLON-ALON(IA+1))**2+(SLAT-ALAT(IB))**2)
       SD(4)=SQRT((SLON-ALON(IA+1))**2+(SLAT-ALAT(IB+1))**2)
       
        SDM1=SD(1)
         M=1
       DO 119 K=2,4
        IF(SD(K).LT.SDM1) THEN
         SDM1=SD(K)
         M=K
        ENDIF
 119   CONTINUE        
...
C** OUTPUT INFORMATION
 ...
 120   FORMAT(1X,'Logitude ',F6.2,'W',5X,
     +   'Latitude ',F6.2,'N')

       WRITE(376,125) ALON(IA),ALAT(IB)
 125   FORMAT(/1X,'Numerical Grids Used (Nearest ponit):'
     +  /1X,'Longitude - ',F6.2,'W',8X,'Latitude  - ',
     +  F6.2,'N',5X)
 ...
 130   FORMAT(1X//1X,'Canadian Climate Centre GCM OUTPUT:'/1X)
       ...
       IF(ICO.EQ.1) THEN
        OPEN(71,FILE='D:\Inetpub\wwwroot\lake\FortranModel\canada1.dat')
       ELSE
        OPEN(71,FILE='D:\Inetpub\wwwroot\lake\FortranModel\canada2.dat')
       ENDIF
 
       IF(ICO.EQ.1) THEN
        OPEN(72,FILE='D:\Inetpub\wwwroot\lake\FortranModel\sco1.dat')
       ELSE
        OPEN(72,FILE='D:\Inetpub\wwwroot\lake\FortranModel\sco2.dat')
       ENDIF
    ...
    6  NCE=0
       ...
       NLAB=NBF
       NR=NR+1
       READ(NBF,'(1X,A4,I10,1X,A4,5I10,10X)',ERR=80)IFID,IPER,
     2  ITYP,ILEV,MDM,NDM
C
       DO 12 II=1,IJDM,5
       IE=II+4
...
C READ THE INCOMING SOLAR RADAITION DATA
         IF(K.EQ.6) THEN
...
C LOOK FOR NEXT HEADER
 ...
 160   CONTINUE
       ...
       IF(ICO.EQ.1) THEN
        ...
        CLOSE (376)
Cfx
        OPEN(376,FILE='D:\Inetpub\wwwroot\lake\FortranModel\co2.dat')
        GOTO 666
       ELSE
        CLOSE (376)
        CLOSE (71)
...
C************************************************C
C
C  Combine the 1xCO2 and 2xCO2 output results
C  to get the difference or ratios
C  (By Xing Fang (June, 1995)
C
C***********************************************C
       SUBROUTINE COMBINE
     ...
     +  FPRE(12),FSOL(12)
       ...
       OPEN(71,FILE='D:\Inetpub\wwwroot\lake\FortranModel\co1.dat')
       OPEN(72,FILE='D:\Inetpub\wwwroot\lake\FortranModel\co2.dat')
...
C***1XCO2 OUTPUT ****************
 ...
 115   FORMAT(17X,A20)
       ...
       OPEN(376,FILE='D:\Inetpub\wwwroot\lake\FortranModel\fco2.dat')
...
C** OUTPUT INFORMATION
 ...
 220   FORMAT(1X,'Logitude ',F6.2,'W',5X,
     +   'Latitude ',F6.2,'N')

       WRITE(376,225) ALON,ALAT
 225   FORMAT(/1X,'Numerical Grids Used (Nearest point):'
     +  /1X,'Longitude - ',F6.2,'W',8X,'Latitude  - ',
     +  F6.2,'N',5X)
 ...
 255      FORMAT(1X,I2,3X,4(5X,F6.2,5X))
         ...
         ELSE
          ...
          IF(K.EQ.2) THEN
C          TP=101330
C          ECO1=CO1(J,K)*TP/0.622
C          ECO1=ALOG(ECO1/611.0)
C          DT1=237.3*ECO1/(17.27-ECO1)
C          ECO2=CO2(J,K)*TP/0.622
C          ECO2=ALOG(ECO2/611.0)
C          DT2=237.3*ECO2/(17.27-ECO2)
C          FDEW(J)=COE(K)*(DT2-DT1)
