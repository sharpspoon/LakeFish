...
C     &	         ' ',' ',' '/,
     ...
     &	         ' ',' ',' '/
      open(17,file="D:\Project\Huang\test\lake\path.txt")
      READ(17,'(A)') RUNPATH
      READ(17,'(A)') METERPATH
c      READ(*,'(A)') RUNPATH
c      READ(*,'(A)') METERPATH
...
C20** annual mean chl-a concentration (CHLMEAN), use a default 
C20** seasonal pattern, SDY - Secchi Disk Depth FOR EVERY YEAR! 
        DO 147 IK=1,NYTOT 
	   READ (7,*) CHLMEAN(IK),SDY(IK)
 147	  CONTINUE
	ENDIF

C ALWAYS READ the chlorophyll-a concentration data
