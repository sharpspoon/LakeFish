...
C... restore initial T & DO data
        IF(YEARBAK.EQ.MYEAR.AND.MONBAK.EQ.MONTH.AND.DAYBAK.EQ.MDAY) THEN
          IF(T_FLAG.EQ.0.OR.DO_FLAG.EQ.0) THEN
             IF(T_FLAG.EQ.1.OR.DO_FLAG.EQ.1) THEN      
...
C.... Julian day must be reset
     ...
     +                     MONBAK,DAYBAK,YEARBAK
          ENDIF
        ELSE
          IF(DO_FLAG.EQ.0) WRITE(198,1984)MONTH,ISTART,MYEAR,MONBAK,
     +                     DAYBAK,YEARBAK,MONBAK,DAYBAK,YEARBAK
          IF(DO_FLAG.EQ.0) WRITE(99,1984)MONTH,ISTART,MYEAR,MONBAK,
     +                     DAYBAK,YEARBAK,MONBAK,DAYBAK,YEARBAK
        ENDIF
...
C.... the mean Temperature of inputted T.
      IF((T_FLAG.EQ.1).AND.(DO_FLAG.EQ.0.AND.RATIOX.GE.5)) THEN
         T2X = 0.0
	   T2MAX = 0.0
	   T2MIN = 100.0
	   DO I = 1,MBOT
...
C.... which is dependent on temperature and eleveation. 
      IF(DO_FLAG.EQ.0.OR.T_FLAG.EQ.0) THEN
	   DO I = 1,MBOT
	      T2X = T2(I)+273.15
	      DOXS = -139.34411+1.575701E5/T2X-6.642308E7/T2X**2.0
     +        +1.2438E10/T2X**3.0-8.621949E11/T2X**4.0
